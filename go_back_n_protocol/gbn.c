...
#define WINDOWSIZE 6    /* (AKA N) alternating bit only allows one unacked packet */
...
bool IsCorrupted(struct pkt packet)
{
  if (packet.checksum == ComputeChecksum(packet)){
    if (TRACE  >5)    
      printf("message %s IS NOT currupted. checksum: %d\n", packet.payload, packet.checksum);
    return (false);
  } else {
    if (TRACE  >5)    
      printf("message %s IS currupted. checksum: %d\n", packet.payload, packet.checksum);
    return (true);
  }
}
...
void A_input(struct pkt packet)
{
  ...
  if (!IsCorrupted(packet)) {
    ...
    if (packet.acknum != prevAck) {    /**** 2. FILL IN CODE replace TRUE with test whether this is a new ACK ***/
      if (TRACE >    0)
        printf("ACKNOWLEDGED acknum: %d\n",packet.acknum);
      /* packet is a new ACK */
      if (TRACE > 0)
        printf("----A: ACK %d is not a duplicate\n",packet.acknum);
      new_ACKs++;
      prevAck = packet.acknum;
      ...
      windowcount = windowcount - accCount;
      windowfirst = (packet.acknum + 1) % WINDOWSIZE; 
      base = (packet.acknum + 1) % (WINDOWSIZE+1);



      printf("wc: %d, wf: %d, wl: %d, base: %d\n", windowcount, windowfirst, windowlast, base);
...
void B_input(struct pkt packet)
{
  ...
  if  ( (!IsCorrupted(packet))  && (packet.seqnum == expectedseqnum) ) {
    ...
    if (TRACE >    0)
        printf("PACKET %d CORRUPTED OR NOT EXPECTED: %d\n",packet.seqnum, expectedseqnum);

    if (TRACE > 0) 
      printf("----B: packet corrupted or not expected sequence number, resend ACK!\n");
    /***** 3. FILL IN CODE  What ACK number should be sent if the packet
	   was corrupted or out of order? *******/ 
     sendpkt.acknum = (expectedseqnum - 1) % (WINDOWSIZE+1);
     sendpkt.acknum = sendpkt.acknum < 0 ? sendpkt.acknum + (WINDOWSIZE+1) : sendpkt.acknum;
  }
